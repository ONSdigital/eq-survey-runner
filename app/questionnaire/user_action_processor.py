...
class UserActionProcessor(object):
    ...
    def _build_user_action_chain(self):
        ...
        remove_person = RemovePerson(self._schema, self._metadata, self._questionnaire_manager)
        ...
        start_questionnaire.set_next_action(remove_person).set_next_action(add_another_person).set_next_action(save_continue).set_next_action(submit)
...
class RemovePerson(UserAction):

    def __init__(self, schema, metadata,questionnaire_manager):
        super(RemovePerson, self).__init__(schema, metadata)
        self.action = 'remove_person'
        self.questionnaire_manager = questionnaire_manager

    def perform_action(self):
        self.questionnaire_manager.remove_person()
