...
from app.questionnaire.create_questionnaire_manager import create_test_survey_manager
...
@main_blueprint.route('/test_survey/<location>', methods=["GET", "POST"])
@login_required
def test_survey(location):
    # Redirect to thankyou page if the questionnaire has already been submitted
    if SubmitterConstants.SUBMITTED_AT_KEY in session and location != 'thank-you':
        return redirect('/test_survey/thank-you')

    questionnaire_manager = create_test_survey_manager()

    if location == 'first':
        questionnaire_manager.go_to_first()
        return redirect('/test_survey/' + questionnaire_manager.get_current_location())
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            return redirect('/test_survey/' + next_location)
