...
class Block:
    def __init__(self, block_schema, answer_store, list_store, metadata, schema):
        self.id = block_schema['id']
        self.title = block_schema.get('title')
        self.number = block_schema.get('number')
        self.link = self._build_link(block_schema['id'])
        self.question = self.get_question(
            block_schema, answer_store, list_store, metadata, schema
        )

    @staticmethod
    def _build_link(block_id):
        return url_for('questionnaire.get_block', block_id=block_id)
    ...
    def get_question(block_schema, answer_store, list_store, metadata, schema):
        ...
        for variant in block_schema.get('question_variants', []):
            display_variant = evaluate_when_rules(
                variant.get('when'), schema, metadata, answer_store, list_store
            )
            if display_variant:
