...
class TestQuestionnaireQuestionVariants(IntegrationTestCase):
    ...
    def complete_first_section(self, proxy=False):
        ...
        expected_question = (
            'What age is <em>Linus Torvalds</em>?' if proxy else 'What is your age?'
        )
        ...
        expected_question = (
            '<em>Linus Torvalds</em> is over 16?' if proxy else 'You are over 16?'
        )
        self.assertInBody(expected_question)
    ...
    def complete_currency_section(self):
        ...
        self.post({'second-number-answer': 321})
