...
import WhoLivesAtThisAddress from '../pages/surveys/census/household/who-lives-at-this-address.page.js'
...
describe('Example Test', function () {
    ...
    it('Given a census household survey, When i enter valid data, Then the survey should submit successfully', function () {
        startQuestionnaire('census_household.json')

        // who-lives-here
        PermanentOrFamilyHome.clickYes().submit()
        WhoLivesAtThisAddress.setWhoLivesAtThisAddressAnswer("John Smith").submit()
        EveryoneAtAddressConfirmation.clickYes().submit()
        OvernightVisitors.setOvernightVisitorsAnswer(0).submit()
        HouseholdRelationships.clickHusbandOrWife().submit()
        WhoLivesHereCompleted.submit()

        // household-and-accommodation
        TypeOfAccommodation.clickWholeHouseOrBungalow().submit()
        TypeOfHouse.clickSemiDetached().submit()
        SelfContainedAccommodation.clickYesAllTheRoomsAreBehindADoorThatOnlyThisHouseholdCanUse().submit()
        NumberOfBedrooms.setNumberOfBedroomsAnswer(3).submit()
        CentralHeating.clickGas().submit()
        OwnOrRent.clickOwnsOutright().submit()
        NumberOfVehicles.setNumberOfVehiclesAnswer(2).submit()
        HouseholdAndAccommodationCompleted.submit()

        // household-member
        HouseholdMemberBegin.submit()
        DetailsCorrect.clickYesThisIsMyFullName().submit()
        Sex.clickMale().submit()
        DateOfBirth.setDateOfBirthAnswerDay(2).setDateOfBirthAnswerYear(1980).submit()
        Over16.clickYes().submit()
        MaritalStatus.clickMarried().submit()
        AnotherAddress.clickNo().submit()
        InEducation.clickNo().submit()
        CountryOfBirth.clickEngland().submit()
        Carer.clickNo().submit()
        NationalIdentity.clickBritish().submit()
        EthnicGroup.clickWhite().submit()
        WhiteEthnicGroup.clickEnglishWelshScottishNorthernIrishBritish().submit()
        SexualIdentity.clickHeterosexualOrStraight().submit()
        UnderstandWelsh.clickNoneOfTheAbove().submit()
        Language.clickEnglish().submit()
        Religion.clickNoReligion().submit()
        PastUsualAddress.clickThisAddress().submit()
        Passports.clickUnitedKingdom().submit()
        Disability.clickNo().submit()
        Qualifications.clickUndergraduateDegree().submit()
        Volunteering.clickNo().submit()
        EmploymentType.clickWorkingAsAnEmployee().submit()
