...
class DateRangeTypeTest(unittest.TestCase):
    ...
    def test_date_range_type_validator(self):
        ...
        self.assertFalse(result.is_valid)
        self.assertEqual(len(result.errors), 1)
        self.assertEqual(AbstractValidator.INVALID_DATE_RANGE_TO_FROM_SAME, result.errors[0])

        # from date before to date
        result = date_range_type.validate(['20/01/2018', '20/02/2016'])
        self.assertFalse(result.is_valid)
        self.assertEqual(len(result.errors), 1)
        self.assertEqual(AbstractValidator.INVALID_DATE_RANGE_TO_BEFORE_FROM, result.errors[0])
